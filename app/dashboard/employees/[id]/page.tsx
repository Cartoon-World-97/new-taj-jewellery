...
export default function ClientDetailPage() {
  ...
  const fetchClientStatement = async () => {
    try {
      ...
      const response = await fetch(`/api/employee-statement?employeeId=${clientId}`, {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingTransaction 
        ? `/api/employee-transactions/${editingTransaction._id}`
  ...
  const handleDelete = async (transactionId: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`/api/employee-transactions?id=${transactionId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },

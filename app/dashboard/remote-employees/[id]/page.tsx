...
export default function ClientDetailPage() {
  ...
  const fetchClientStatement = async () => {
    try {
      ...
      const response = await fetch(`/api/remote-employee-statement?employeeId=${clientId}`, {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingTransaction 
        ? `/api/remote-employee-transactions/${editingTransaction._id}`
        : '/api/remote-employee-transactions';
  ...
  const handleDelete = async (transactionId: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`/api/remote-employee-transactions?id=${transactionId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          ...
          <Link href="/dashboard/remote-employees" className="btn-primary">
            Back to Remote Employees
          </Link>
        </div>
      </div>
    );

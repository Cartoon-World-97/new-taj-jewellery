...
export async function POST(request: NextRequest) {
  try {
    ...
    // Insert transaction
    const result = await db.collection('clientsTransactions').insertOne(newTransaction);
  ...
  try {
    ...
    if (txn.employeeId) {
      ...
      if (employeeTransactions.length === 0) {
        ...
        await db.collection('remoteEmployees').updateOne(
    ...
    return NextResponse.json({
      message: 'Clients transaction deleted successfully',
    });
  } catch (error) {
